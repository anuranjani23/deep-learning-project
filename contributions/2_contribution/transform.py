...
class PatchRotation:
    ...
    def __call__(self, img_np: np.ndarray) -> np.ndarray:
        ...
        if img_np.shape[:2] != self.output_size:
            return cv2.resize(img_np, (self.output_size[1], self.output_size[0]), interpolation=self.interpolation)
        return img_np
...
def get_transform(
    ...
    if transform_names == ['none'] or transform_names == ['']:
        transform_names = []
    ...
    for transform_name, p in zip(transform_names, augment_ps):
        ...
        if 'patchrotation' == transform_name:
            output_size = (resize_size, resize_size) if resize_size is not None else (224, 224)
            compose += [PatchRotation(grid_size=grid_size, p=p, output_size=output_size)]

        if 'randomresizedcrop' == transform_name:
            if resize_size is not None:
                compose += [RandomResizedCrop((resize_size, resize_size), scale=(0.3, 1.0))]
                has_explicit_resize = True
    ...
    if not has_explicit_resize and resize_size is not None:
        compose = [Resize((resize_size, resize_size))] + compose
