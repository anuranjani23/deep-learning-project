...
# Run 4 STL10 finetunes (baseline + shape/texture/color removed) with up to 3 concurrent jobs.
# Then run inference for each model in one go.
...
# Filter GPU_LIST by actually available GPUs (if nvidia-smi is present)
